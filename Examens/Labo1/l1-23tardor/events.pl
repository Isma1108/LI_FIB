...
%% new for this exam:
enforceMaxDaysPerModerator:- moderator(M), findall(md(M, D), day(D), Lits), maxDaysPerModerator(Max), atMost(Max, Lits), fail.
enforceMaxDaysPerModerator.

enforceNotSameDayModerators:- notSameDayModerators(M1, M2), day(D), writeOneClause([-md(M1, D), -md(M2, D)]), fail.
enforceNotSameDayModerators.

enforceBefore:- before(E1, E2), day(D1), day(D2), D1 < D2, writeOneClause([-ed(E2,D1), -ed(E1,D2)]), fail.
enforceBefore.
