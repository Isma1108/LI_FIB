...
writeClauses :-
    eachWorkerInExactlyOneTeam,
    correctSizeInEachTeam,
    allPairsCompatible,
    true,!.
writeClauses :- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachWorkerInExactlyOneTeam :- worker(W), findall(wt(W,T), team(T), Lits), exactly(1, Lits), fail.
eachWorkerInExactlyOneTeam.

correctSizeInEachTeam :- team(T), findall(wt(W,T), worker(W), Lits), minSize(N), maxSize(M), atLeast(N, Lits), atMost(M, Lits), fail.
correctSizeInEachTeam.

allPairsCompatible :- incompatibleWorkers(W1, W2), team(T), writeOneClause([-wt(W1,T), -wt(W2,T)]), fail.
allPairsCompatible.
