...
%No hace falta una variable SAT dcc para la restriccion de la frecuencia de dias a la semana,
%ya que sabemos que en un solo dia el viaje se hace como mucho una vez, por lo que nos da igual
%tener rondando por el predicado la variable B bus

%Aunque no tiene sentido usarlo si entendemos bien el problema...
%Si quisieramos usar la variable SAT dcc (el dia D se hace el trip C1-C2), entonces 
%el predicado correctFrequenciesTrip seria:
%correctFrequenciesTrip :- trip(C1-C2), frequency(C1-C2, F),
%   findall(dcc(D,C1,C2), day(D), Lits), atLeast(F, Lits), fail.
%correctFrequenciesTrip.
%
%Pero luego seria necesario "linkar" las dos variables SAT: si algun bus hace un trayecto
%un dia, entonces dcc del dia debe ser cierto, y si algun dia se hace un trayecto, algun bus 
%debe hacerlo ese dia: es un si y solo si! (<->). Podemos hacer las dos implicaciones a mano,
%o usar expressOr, un predicado de la libreria que nos facilita el trabajo:
%
%linkSatVars :- day(D), trip(C1-C2), findall(bdcc(B,D,C1,C2), bus(B), Lits),
...
writeClauses :- 
    ...
    restrictionOriginDestination,
    tripAtMostOncePerDay,
    correctFrequenciesTrip,
    limitDistanceTwoConsecutiveDays,
    true,!.
...
restrictionOriginDestination :-
  bus(B), consecutiveDays(D1, D2), trip(C1-C2), trip(C3-C4), C2 \= C3,
...
limitDistanceTwoConsecutiveDays :-
  bus(B), consecutiveDays(D1, D2), tooLongDist(C1-C2-C3),
  writeOneClause([-bdcc(B,D1,C1,C2), -bdcc(B,D2,C2,C3)]), fail.
limitDistanceTwoConsecutiveDays.
