...
%% Si, necesitamos una SAT var para evitar generar un numero abusivo de clausulas en el predicado
% que controla el límite de torres a colocar. Por ejemplo, si solo tenemos towerPos, si queremos
% decir que como maximo queremos 10 torres, aunque hagamos un atMost(10, Lits) donde Lits son 
% todas aquellas towerPos(I,J) tales que posVillage(_,I,J), imaginando que |Lits| = N, tenemos
% binomial(N,10) clausulas solo para este predicado, es demasiado!
%
% Es obvio que la libreria que se nos da usa la forma naive de traducir atLeast y atMost (con coste polinomico),
% pero aunque usemos una forma mas astuta seguirian siendo demasiadas clausulas. Es mejor usar una variable 
% SAT auxiliar que controla si un village tiene torre o no. Asi, ahora para el atMost pasamos a tener 
% binom(N', 10) clausulas, donde N' ahora es el numero de villages, mucho menos que antes!.

satVariable( towerVillage(V) ) :- village(V). 

%% Hay que recordar ligar las variables! towerPos(I,J) <-> towerVillage(V) donde V es el village de (I,J)
%% Habra un predicado para el "link"
...
writeClauses(MaxNumTowers):-
    ...
    allVillageWatched,        
    allSignificantWithTower,
    allTowersInVillage,
    atMostUpperLimitTowers(MaxNumTowers),
    linkVars,
...
allSignificantWithTower :-
    significantVillage(V), writeOneClause([towerVillage(V)]), fail.
allSignificantWithTower.

allTowersInVillage :-
    position(I,J), \+ posVillage(_,I,J), writeOneClause([-towerPos(I,J)]), fail.
allTowersInVillage.

atMostUpperLimitTowers(MaxNumTowers) :-
    findall(towerVillage(V), village(V), Lits), atMost(MaxNumTowers, Lits), fail.
atMostUpperLimitTowers(_).
