...
noIncompatibleDatesAtSameSlot(MaxSlots):- date(P1-P2), date(P3-P4), shareParticipant(P1-P2, P3-P4), slot(S), between(1,MaxSlots,S),
                                        writeOneClause([-pps(P1-P2,S), -pps(P3-P4,S)]), fail.
noIncompatibleDatesAtSameSlot(_).

relatePSwithPPS(MaxSlots):- participant(P), slot(S), between(1,MaxSlots,S), findall(pps(P-P1,S), date(P-P1), Lits1), 
                            findall(pps(P1-P,S), date(P1-P), Lits2), append(Lits1, Lits2, Lits), expressOr(ps(P,S), Lits), fail.
relatePSwithPPS(_).

restEveryKConsecutiveDates(MaxSlots):- maxConsecutiveDates(K), K1 is K+1, kConsecutiveSlots(MaxSlots, K1, LS),
                                    participant(P), findall(ps(P,S), member(S,LS), Lits), negateAll(Lits, NLits), atLeast(1, NLits), fail.
restEveryKConsecutiveDates(_).

maxDatesAtTheSameSlot(MaxSlots):- numTables(T), slot(S), between(1,MaxSlots,S), findall(pps(P1-P2,S), date(P1-P2), Lits), atMost(T, Lits), fail.
...
costOfThisSolution(M,Cost):- findall(S, member(ps(_,S), M), L), sort(L, Ls), length(Ls, Cost), !.
