...
pert_con_resto(X,L,R):- concat(L1,[X|L2], L    ),  % L es la concatenacion de un L1 cualquiera seguido de X seguido de un L2 cualquiera
                        concat(L1,   L2,  R).      % R es la concatenacion de de todo menos X
...
union( [X|L1], L2, [X|U] ):-     \+member(X,L2), union( L1, L2, U ).
