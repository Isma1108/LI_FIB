...
%  means:  "gangster G does task T at hour H"     (MANDATORY)
...
writeClauses(MaxConsecutiveHours) :-
    ...
    sameTaskConsecutiveHours,
    restrictNotAvailable,
...
allTasksDone :-
    ...
    findall(does(G,T,H), gangster(G) , Lits), exactly(N, Lits), fail.
allTasksDone.

atMostOneTaskPerHour :-
    available(G,H), findall(does(G,T,H), task(T), Lits), atMost(1, Lits), fail.  
atMostOneTaskPerHour.
...
atMostMaxConsecutiveHours(MaxConsecutiveHours) :-
    gangster(G), task(T),
    %% Miramos un rango de Max + 1 horas consecutivas
    hour(Hstart), hour(Hend), Hend is Hstart + MaxConsecutiveHours,
    findall(does(G,T,H), between(Hstart, Hend, H), Lits), 
    %% Lo que queremos es que almenos una de esas horas no se haga nada,
    %% que es equivalente a ver que almenos una de las negadas se hace algo
    %% No hace falta mirar esto para todo rango mayor que max, ya que si no se
    %% cumple para max + 1, tampoco para max + 2 ...
...
%% Este es el computeCost mas dificil del Lab4. Para computar el coste podemos
%% partir de un hipotetico coste maximo (72), e ir consultando si para alguna lista de horas
%% trabajadas de un gangster existe una sublista de horas consecutivas de exactamente Cost elementos.
%% Si no existe, bajamos el coste en uno, y lo vamos haciendo hasta encontrar una sublista de tamaño
%% ese coste, que seria el coste de nuestra solucion

costOfThisSolution(M,Cost) :- 
    between(1,72,N), Cost is 73 - N,
    gangster(G), findall(H, member(does(G,_,H), M), L), sort(L, Hours),
    sublistWithCost(Hours, Cost), !.

sublistWithCost(L, Cost) :-
    append(_, Sublist, L), length(Sublist, Cost),
    consecutiveHours(Sublist).

consecutiveHours([]).
consecutiveHours([_]) :- !.
consecutiveHours([H1,H2|S]) :- H2 is H1 + 1, consecutiveHours([H2|S]).
