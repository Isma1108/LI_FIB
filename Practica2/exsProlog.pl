...
%Utilitzo els ! per evitar probar alternativa fib(N,F) quan estem al cas base
% Quan N es > 5 no hi ha alternatives per les dues crides recursives de fib
% en els dos casos l'unica alternativa torna a ser fib(N, F).
% deixo N > 2 com a condicio per si arriben entrades del tipus fib(-1, F),
% en aquell cas si no hi ha un N > 2 tenim recusio infinita => stack overflow :(
...
%Necesitem un predicat auxiliar per poder tenir una variable amb la llista de 
%parelles i fer-ne un write.


card_aux([], []).
card_aux([X|L], [[X,N]|S]):- card_aux(L,R), select([X,N1],R,S), N is N1 + 1, !.
card_aux([X|L], [[X,1]|S]):- card_aux(L,S).
...
esta_ordenada([]).
esta_ordenada([_]).
esta_ordenada([X1,X2|L]):- esta_ordenada([X2|L]), X1 =< X2. 
...
%Les repeticions es donen quan tenim elements repetits, perque permutation
%no ho te en compte, interpreta cada element com unic independentment del valor

%Podria ser interessant intentar evitar les repeticions mes endevant

palindroms(L):- permutation(L, P), reverse(P, R), P = R, write(P), nl, fail.
...
ok([]).
ok([_]).
ok([f(_,X1), f(X2,X3)|L]):- X1 = X2, ok([f(X2,X3)|L]).


%El predicat p inicial es el mateix que permutation
p([],[]).
p(L,[X|P]) :- select(X,L,R), p(R,P).
%Pero afegim la posibilitat de girar fitxes
p(L,[f(X1,X2)|P]):- select(f(X2,X1), L, R), p(R,P).
...
%He pogut comprobar ràpidament que no es pot donar el cas plantejat
%Es podria fer l'analisi matemàtic i en principi s'arribaria a la mateixa
%conclusió
...
main :-
    ...
    between(0,3,NSC2),   % NSC2:  "no. non-smokers with cancer group 2"
    between(0,3,NSNC2),  % NSNC2: "no. non-smokers with no cancer group 2"
    10 is SC2 + SNC2 + NSC2 + NSNC2,
    S1 is SC1 / (SC1 + SNC1), NS1 is NSC1 / (NSC1 + NSNC1), S1 > NS1,
    S2 is SC2 / (SC2 + SNC2), NS2 is NSC2 / (NSC2 + NSNC2), S2 > NS2,
    TS is (SC1 + SC2) / (SC1 + SC2 + SNC1 + SNC2),
    TNS is (NSC1 + NSC2) / (NSC1 + NSC2 + NSNC1 + NSNC2),
    TNS > TS.
